...
{
    public class CreateAccessLevelCommandHandler : IRequestHandler<CreateAccessLevelCommand, Result>
    {
        private readonly IDeviceManagerContext _context;
        private readonly ILocalization _localization;

        public CreateAccessLevelCommandHandler(IDeviceManagerContext context, ILocalization localization)
        {
            _context = context;
            _localization = localization;
        }

        public async Task<Result> Handle(CreateAccessLevelCommand request, CancellationToken cancellationToken)
        {
            #region Find Device
            ...
            var result = context.SetAccessLevel(new BaseDeviceInfoDto
            {
                Ip = device.Ip,
                Serial = device.Serial,
                Port = device.Port,
                Code = device.DeviceModel.Code
            }, request.CreateAccessLevel);
